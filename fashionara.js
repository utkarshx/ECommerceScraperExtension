...
function reportPurchase(){
  ...
  if(curURL.split('fashionara.com/onestepcheckout').length>1){
    chrome.runtime.sendMessage({processDONE: "Fashionara"}, function(response) {
    });
  }
}
...
var myurl = "http://compare.buyhatke.com/addWatchList.php";
         ...
         if($('[itemprop="lowPrice"]').text().split("Rs.").length > 1)
          {
            var price = $('[itemprop="lowPrice"]').text().split("Rs.")[1].split(",").join("").trim();
          }
          else if($('[itemprop="lowPrice"]').length > 0)
          {
            var price = $('[itemprop="lowPrice"]').text().split(",").join("").trim();
          }
          else if($('.special-price').text().split(",").join("").split('Rs.').length > 1)
          {
            var price = $('.special-price').text().split(",").join("").split('Rs.')[1].trim();
          }
...
var parameters =  encodeURIComponent(prod) + "~*~*" + myPrice + "~*~*" + encodeURIComponent(image2) + "~*~*" + encodeURIComponent(url) + "~*~*98" ;
...
button.addEventListener("click", function(){
  ...
  document.getElementById('bhWidget').innerHTML = '<div class="line fk-font-12" style="margin-bottom: 4px;">Thank You ! We appreciate your motive to save energy :)</div>';
      ...
      if($('.shop-box').length>0){
      $('.shop-box').before(strToDisp);
      }
      else if($('.add-to-cart').length>0){
        $('.add-to-cart').after(strToDisp);
      }
      var button = document.getElementById("removeMe2");
button.addEventListener("click", function(){
  removeAlert();
...
if(flagToDisp==0){
  var imgURL2 = chrome.extension.getURL("watch-price1.png");
if($('.shop-box').length>0){
$('.shop-box').before('<div id="bhWidget"><a id="addWatchList" style="margin-top: 4px;" alt="Add to Watch List" title="Add to BuyHatke Watch List and get notifications on price drop" href="javascript:void();" class="fk-inline-block buy-btn fksk-buy-btn"><img style="margin-left:-12px;" src=' + imgURL2 +'></a></div>');
 }
 else if($('.add-to-cart').length>0){
  $('.add-to-cart').after('<div id="bhWidget"><a id="addWatchList" style="margin-top: 4px;" alt="Add to Watch List" title="Add to BuyHatke Watch List and get notifications on price drop" href="javascript:void();" class="fk-inline-block buy-btn fksk-buy-btn"><img style="margin-left:-12px;" src=' + imgURL2 +'></a></div>');
 }
...
var title = $('.product-name:eq(0)').find('h1').text().trim();
if($('.brand-logo').length>0){
  var title2 = $('.brand-logo').find('img').attr("title");
  title = title.trim();
}
origProd = title;
title = title.split(" ");
...
var theKid= '<a alt="Compare via Compare Hatke" title="Compare via Compare Hatke" target="_blank" href="' + urlToFollow + '"><button type="button" id="compare_now_button" class="button-large validation-passed"><span><span>Compare</span></span></button></a>';

$('#add_to_cart_button').after(theKid);

var price = $('.price-box:eq(0)').text().trim().split('Rs.');
price = price[price.length-1].trim();
price = price.split(",").join("");
myPrice = price;

var final2send = urlToFollow.split("products/");
var msgToSend = final2send[1] + "~*~*" + myPrice + "&moreData=null&isapparel=1";
