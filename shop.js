...
var name = $s('.product-name h1').text().trim();
                var price = $s('.add-to-cart .price').text().split("Rs.");
                price = price[price.length-1];
                price = price.trim().split(",").join("");
                if(price!=""){
                if($s(".special-price").length > 0)
                {
                  var price = $s('.special-price .price').text().split("Rs.")[1].split(",").join("").trim();
                }
                else if($s('.price-box').length > 0){
                  if($s('.price-box').clone().find('span').text().split("Rs.").length > 1)
                  {
                    var price = $s('.price-box').clone().find('span').text().split("Rs.")[1].split(",").join("").trim();
                  }
                  else
                  {
                    var price = $s('.price-box').clone().find('span').text().split(",").join("").trim();
                  }
                }
                else
                {
                ...
                var image_url = $s('#imgshow').attr('value');
...
var parameters =  encodeURIComponent(prod) + "~*~*" + myPrice + "~*~*" + encodeURIComponent(image2) + "~*~*" + encodeURIComponent(url) + "~*~*45" ;
...
button.addEventListener("click", function(){
  ...
  document.getElementById('bhWidget').innerHTML = '<div class="line fk-font-12" style="margin-bottom: 4px;">Thank You ! We appreciate your motive to save energy :)</div>';
      ...
      if($s('.priceCombo').length>0){
      $s('.priceCombo').before(strToDisp);
      }
...
if(flagToDisp==0){
  var imgURL2 = chrome.extension.getURL("watch-price1.png");
if($s('.priceCombo').length>0){
$s('.priceCombo').before('<div id="bhWidget"><a id="addWatchList" style="margin-top: 4px;" alt="Add to Watch List" title="Add to BuyHatke Watch List and get notifications on price drop" href="javascript:void();" class="fk-inline-block buy-btn fksk-buy-btn"><img style="margin-left:-12px;" src=' + imgURL2 +'></a></div>');
 }
 else if($s('.price-box').length>0 && $s('.price-box').length == 1){
...
name = nameS.join("-");
 	...
 	name = nameS[0] + "-" + nameS[1] + "-" + nameS[2] + "-" + nameS[3] + "-" + nameS[4] + "-" + nameS[5] + "-" + nameS[6] + "-" + nameS[7];
 }
var url = "http://compare.buyhatke.com/products/" + name;
var imgURL = chrome.extension.getURL("sstop.png");
$s('#addtoCart_div').after('<a target="_blank" href=' + url + '><img src='+ imgURL +' alt="Compare via Compare Hatke" style="margin-left: -7px;"></a>');
...
var msgToSend = final2send[1] + "~*~*" + price + "moreData=isapparel=1";
