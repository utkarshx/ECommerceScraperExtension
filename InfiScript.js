...
function reportPurchase(){
  ...
  if(curURL.split('infibeam.com/ShowCart.action').length>1){
    chrome.runtime.sendMessage({processDONE: "Infibeam"}, function(response) {
    });
...
function sendPairs(){
  ...
  for(i=0;i<sliderLength;i++){
    var countFurther = $('.boxinner').find('li').length;
    for(j=0;j<countFurther;j++){
    var url = $('.boxinner:eq(' + i + ')').find('li:eq(' + j + ')').find('a:eq(0)').attr("href");
    if(url!= undefined){
      url2 = url.split("?");
      PID = url2[0];
      if(PID.split(parent).length==1){
        PID = "http://" + parent + PID;
      }
      var price = $('.boxinner:eq(' + i + ')').find('li:eq(' + j + ')').find('.price');
      if(price.length>0){
        price = $('.boxinner:eq(' + i + ')').find('li:eq(' + j + ')').find('.price').find('.normal').text().trim();
  ...
  var sliderLength = slider.length;
  for(i=0;i<sliderLength;i++){
    var url = $("#irec").contents().find('.footer-item:eq(' + i + ')').find('a:eq(0)').attr("href");
      url2 = url.split("?");
      PID = url2[0];
      if(PID.split(parent).length==1){
        PID = "http://" + parent + PID;
      }
      var price = $("#irec").contents().find('.footer-item:eq(' + i + ')').find('.price');
      if(price.length>0){
        price = $("#irec").contents().find('.footer-item:eq(' + i + ')').find('.price').text().trim().split(" ")[0].split("Rs.")[1].trim();
        price = price.split(",").join("");
        price = parseFloat(price);
      }
      if(typeof(price) == "number"){
  ...
  chrome.runtime.sendMessage({pairsInfi: arrayToSend}, function(response) {
    });
}
...
var parameters = "ext_id=" + ext_id + "&auth_val=" + ext_auth + "&pos=10";
...
coupMore = $('#festive-offer-message').text();
if(coupMore!=""){
  coupMore = coupMore.split("coupon code");
  if(coupMore.length>1){
    coupMore = coupMore[1];
    coupMore = coupMore.trim();
...
function checkSavings(){
  $('.hatke-discount-cover:eq(0)').css("display", "block");
  totLen = $('.save-rs').length;
  savings = 0;
  if(totLen==0){
    savings = 0;
  }
  else {
    savings = $('.save-rs:eq(0)').text().trim().split("R")[1].trim();
    savings = parseFloat(savings);
  }
  ...
  if(savings==0&&removeMode==0){
    varName = "savingsInfi" + getCookie("doneTill");
    ...
    document.getElementsByClassName('promo-names-summary')[0].getElementsByTagName('a')[0].click();
    console.log("savings is " + savings);
  }
  else {
    setCookie("removeMode", 0, 1);
...
if(curURL.split('infibeam.com/ShowCart').length>1){
  ...
  if($('#total-cart-cost').length>0){
  $('#total-cart-cost').after("<a id='couponClick' href='javascript:void();'><img style='margin-left:65px;' src='" + imgURL + "'></a>");
  addToDOM();
...
chrome.runtime.sendMessage({detailArray: "haiKya"}, function(response) {
   ...
   filterURL = filterURL.split("#")[0];
   filterURL = filterURL.split("?")[0];
if($('#infiPrice').length>0){
var myPrice  = document.getElementById('infiPrice').innerHTML;
}
else if($('.final-price').length>0){
  var myPrice = $('.final-price').find('meta[itemprop=price]').attr("content");
}
else {
var myPrice = $('.buy-value').html().split("</span>")[1].split(",").join("").trim();
}
...
if($('img[itemprop="image"]').length > 0)
        {
          var image_url = $('img[itemprop="image"]').attr('content');
        }
        else if($('#product-images').length > 0)
        {
          var image_url = $("#product-images a").attr("href");
        }
        else
        {
        ...
        image = image_url;
var parameters =  encodeURIComponent(prod) + "~*~*" + myPrice + "~*~*" + encodeURIComponent(image) + "~*~*" + encodeURIComponent(url) + "~*~*8" ;
...
var title = $('#ib_details').find('h1').text();
if($('#ib_details').find('h1').length==0){
...
var imgURL = chrome.extension.getURL("comp_infi.png");
myParent.innerHTML = "<img src='" + imgURL + "' alt='Compare via Compare Hatke' title='Compare via Compare Hatke'>";
document.getElementById("quantitySelect").parentNode.appendChild(myParent);


  if(flagToDisp==0){
    if($('#priceDiv').length>0){
$('#priceDiv').after('<div id="bhWidget"><a id="addWatchList" style="margin-top: 10px;margin-left:-10px;" alt="Add to Watch List" title="Add to BuyHatke Watch List and get notifications on price drop" href="javascript:void();" class="fk-inline-block buy-btn fksk-buy-btn"><img src=' + imgURL2 +'></a></div>');
 }
 else if($('#pricing_summary').length>0){
  $('#pricing_summary').after('<div id="bhWidget"><a id="addWatchList" style="margin-top: 10px;margin-left:-10px;" alt="Add to Watch List" title="Add to BuyHatke Watch List and get notifications on price drop" href="javascript:void();" class="fk-inline-block buy-btn fksk-buy-btn"><img src=' + imgURL2 +'></a></div>');
 }
