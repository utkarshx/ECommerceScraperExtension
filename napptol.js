...
var name = $('.productDetails').find('h1').text().trim();
name = name.split("(")[0];
origProd = name;
...
var imgURL = chrome.extension.getURL("naaptol.png");

$('.clickToBuy').find('a').after('<a target="_blank" href=' + url + '><img style="margin-left:-3px;" src=' + imgURL +' title="Compare via Compare Hatke"></a>');

var final2send = url.split("products/");
var msgToSend = final2send[1] + "~*~*" + $('.pro_PriceInfo').find('strong:eq(0)').text().trim().split(",").join("");
...
$(document).ready(function(){
  ...
  var origPrice = $('.pro_PriceInfo').find('strong:eq(0)').text().trim().split(",").join("");
