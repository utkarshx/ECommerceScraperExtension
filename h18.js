...
function reportPurchase(){
  ...
  if(curURL.split('homeshop18.com/shopping-cart').length>1){
    chrome.runtime.sendMessage({processDONE: "HomeShop18"}, function(response) {
    });
  }
}
...
var parameters = "ext_id=" + ext_id + "&auth_val=" + ext_auth + "&pid=" + pid + "&web=homeshop18";
...
$(function () {
  //////console.log(dataString);
    $('#container2').highcharts({
      ...
      subtitle: {
        text: 'Price Variance of ' + $('.product-info').find('h1').text().trim() + " at HomeShop18"
      },
      xAxis: {
        type: 'datetime',
...
function sendPairs(){
  ...
  for(i=0;i<sliderLength;i++){
    var countFurther = $('.bx-viewport').find('li').length;
    for(j=0;j<countFurther;j++){
    var url = $('.bx-viewport:eq(' + i + ')').find('li:eq(' + j + ')').find('a:eq(0)').attr("href");
    if(url!= undefined){
      if(url.split("youtube").length>1){
        var url = $('.bx-viewport:eq(' + i + ')').find('li:eq(' + j + ')').find('a:eq(1)').attr("href");
      }
      if(url.split("youtube").length>1){
        var url = $('.bx-viewport:eq(' + i + ')').find('li:eq(' + j + ')').find('a:eq(2)').attr("href");
      }
      url2 = url.split("?");
      PID = url2[0];
      var price = $('.bx-viewport:eq(' + i + ')').find('li:eq(' + j + ')').find('.price');
      if(price.length>0){
        price = $('.bx-viewport:eq(' + i + ')').find('li:eq(' + j + ')').find('.price').html().trim().split("&nbsp;");
  ...
  var sliderLength = slider.length;
  for(i=0;i<sliderLength;i++){
    var countFurther = $('.product-slider').find('li').length;
    for(j=0;j<countFurther;j++){
    var url = $('.product-slider:eq(' + i + ')').find('li:eq(' + j + ')').find('a:eq(0)').attr("href");

    //console.log(url);
    //console.log($('.product-slider').html());
    if(url!= undefined){
      if(url.split("youtube").length>1){
        var url = $('.product-slider:eq(' + i + ')').find('li:eq(' + j + ')').find('a:eq(1)').attr("href");
      }
      if(url.split("youtube").length>1){
        var url = $('.product-slider:eq(' + i + ')').find('li:eq(' + j + ')').find('a:eq(2)').attr("href");
      }
      url2 = url.split("?");
      PID = url2[0];
      var price = $('.product-slider:eq(' + i + ')').find('li:eq(' + j + ')').find('.price');
      if(price.length>0){
        price = $('.product-slider:eq(' + i + ')').find('li:eq(' + j + ')').find('.price').html().trim().split("&nbsp;");
        price = price[price.length-1];
        price = price.split(",").join("");
        price = parseFloat(price);
      }
  ...
  var slider = $('#searchResultsDiv').find('.product_div');
  var sliderLength = slider.length;
  for(i=0;i<sliderLength;i++){
    var url = $('#searchResultsDiv').find('.product_div:eq(' + i + ')').find('a:eq(0)').attr("href");
    if(url!= undefined){
      if(url.split("youtube").length>1){
      var url = $('#searchResultsDiv').find('.product_div:eq(' + i + ')').find('a:eq(1)').attr("href");
      }
      if(url.split("youtube").length>1){
      var url = $('#searchResultsDiv').find('.product_div:eq(' + i + ')').find('a:eq(2)').attr("href");
      }
      url2 = url.split("?");
      PID = url2[0];
      var price = $('#searchResultsDiv').find('.product_div:eq(' + i + ')').find('.price');
      if(price.length>0){
        price = $('#searchResultsDiv').find('.product_div:eq(' + i + ')').find('.price').html().trim().split("&nbsp;");
        price = price[price.length-1];
        price = price.split(",").join("");
        price = parseFloat(price);
      }
  ...
  chrome.runtime.sendMessage({pairsHS18: arrayToSend}, function(response) {
    });
}
...
function preProcessor(i, coupon){
  ...
  $('#code').val(coupon);
  document.getElementsByClassName("cssbtn-orange")[0].click();
...
var parameters = "ext_id=" + ext_id + "&auth_val=" + ext_auth + "&pos=9";
httpq4.open("POST", myurl, true);
httpq4.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
httpq4.onreadystatechange = function(){
if (httpq4.readyState == 4) {
...
function checkSavings(){
  ...
  var totLen = $('.item-head').length;
  for(i=0;i<totLen;i++){
    var curHead = $('.item-head:eq(' + i + ')').text();
    if(curHead.split("Coupon").length>1){
      var curValue = $('.item-head:eq(' + i + ')').nextUntil('f-right').text();
  ...
  if(savings>0){
    varName = "savingsHS18" + getCookie("doneTill");
    ...
    console.log("Savings is 0");
    setCookie("doneTill", parseFloat(getCookie("doneTill")) + 1 , 1);
    console.log("Cookie List is " + getCookie("coupList"));
    console.log("Done till " + getCookie("doneTill"));
    applyCoupons(getCookie("coupList"));
  }
}
...
if(curURL.split('homeshop18.com/checkout').length>1){
  ...
  if($('#applyDiscountForm').length>0){
  $('#applyDiscountForm').after("<a id='couponClick' href='javascript:void();'><img style='margin-left:65px;' src='" + imgURL + "'></a>");
  addToDOM();
...
var myPrice  = $('#hs18Price').html().split("&nbsp;")[1].split(",").join("");
}
...
var prod = $('.product-info').find('h1:eq(0)').text().trim();
...
var img = $('meta[property="og:image"]').attr('content');
          ...
          if(count > 2)
          {
            var image_url = "http:" + img.split("http:")[2];
          }
          else
          {
            var image_url = img;
          ...
          image = image_url;
var parameters =  encodeURIComponent(prod) + "~*~*" + myPrice + "~*~*" + encodeURIComponent(image) + "~*~*" + encodeURIComponent(url) + "~*~*12" ;
...
var name = $('.product-info').find('h1:eq(0)').text().trim();
name = name.split("(")[0];
origProd = name;
...
var imgURL = chrome.extension.getURL("hs18.png");
var imgURL2 = chrome.extension.getURL("watch-price1.png");


if($('#btnBuyNowC').length>0){
  if(flagToDisp==0){
$('#btnBuyNowC').before('<div id="bhWidget"><a id="addWatchList" style="margin-top: 10px;margin-left:-10px;" alt="Add to Watch List" title="Add to BuyHatke Watch List and get notifications on price drop" href="javascript:void();" class="fk-inline-block buy-btn fksk-buy-btn"><img src=' + imgURL2 +'></a></div>');
...
if(parseInt(results[indexSelected].price)<=parseInt(origPrice)&&notFound==0&&document.getElementById('detailOutWrap') == null) {
   ...
   $('.navigation-all').css("margin-top", "10px");
  $("#dd_menu_list").hover(
  function() {
    msg2 = "Results Viewed";
...
button.addEventListener("click", function() {
  ...
  $('.navigation-all').css("margin-top", "0px");
