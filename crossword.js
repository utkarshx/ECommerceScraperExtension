...
$('#cart').find('input:eq(1)').after('<a target="_blank" href=' + url + '><input class="button" name="commit" title="Compare via Compare Hatke" style="width: 78px!important;" value="Compare"></a>');

var final2send = url.split("products/");
var msgToSend = final2send[1] + "~*~*" + $('.final-price').text().split("R")[1].split(",").join("").trim();
...
chrome.runtime.onMessage.addListener(
 function(request, sender) {
  ...
  var origPrice = $('.final-price').text().split("R")[1].split(",").join("").trim();
