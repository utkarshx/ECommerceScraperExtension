...
var name = $('.primary h1').text().trim(); 
                 if($('.price-current').length > 0){
                  if($('.price-current').text().split("Rs").length > 1)
                  {
                    var price = $('.price-current').text().split("Rs")[1].split(",").join("").trim();
                  }
                  else
                  {
                    var price = $('.price-current').text().split(",").join("").trim();
                  }
                }
                else
                {
                ...
                var image_url = $('.imagemain').attr('href');
...
var parameters =  encodeURIComponent(prod) + "~*~*" + myPrice + "~*~*" + encodeURIComponent(image2) + "~*~*" + encodeURIComponent(url) + "~*~*51" ;
...
if(flagToDisp==0){
  var imgURL2 = chrome.extension.getURL("watch-price1.png");
if($('.pricebox').length>0){
$('.pricebox').before('<div id="bhWidget"><a id="addWatchList" style="margin-top: 4px;" alt="Add to Watch List" title="Add to BuyHatke Watch List and get notifications on price drop" href="javascript:void();" class="fk-inline-block buy-btn fksk-buy-btn"><img style="margin-left:-12px;" src=' + imgURL2 +'></a></div>');
 }
 else if($('.price-box').length>0 && $('.price-box').length == 1){
...
var imgURL = chrome.extension.getURL("landmark.png");
$('.buy').after('<a title="Compare via Compare Hatke" target="_blank" href=' + url + '><img src=' + imgURL + ' title="Compare via Compare Hatke"></a>');

var final2send = url.split("products/");
var msgToSend = final2send[1] + "~*~*" + parseFloat($('.price-current').text().split("Rs")[1].split(",").join(""));
...
chrome.runtime.onMessage.addListener(
 function(request, sender) {
  ...
  var origPrice = parseFloat($('.price-current').text().split("Rs")[1].split(",").join(""));
