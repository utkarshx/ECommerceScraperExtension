...
function postProcessor(coupon, i){
   ...
   var msg = $('#promoCodeText').text().trim();
...
var parameters = "ext_id=" + ext_id + "&auth_val=" + ext_auth + "&pos=17";
...
if(curURL.split('secure.yatra.com').length>1){
  ...
  if($('#promoValForm').length>0){
  $('#promoValForm').after("<a id='couponClick' href='javascript:void();'><img style='margin-top:15px;margin-left:20px;' src='" + imgURL + "'></a>");
  addToDOM();
