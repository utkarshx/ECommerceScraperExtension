...
function reportPurchase(){
  ...
  if(curURL.split('zivame.com/checkout/cart/').length>1){
    chrome.runtime.sendMessage({processDONE: "Zivame"}, function(response) {
    });
  }
}
...
var myurl = "http://compare.buyhatke.com/addWatchList.php";
 var name = $s('.proTitle').text().trim();
          if($s('.proPrice').text().split("Rs.").length > 1)
          {
            var price = $s('.proPrice').text().split("Rs.")[1].split(",").join("").trim();
          }
          else
          {
            var price = $s('.proPrice').text().split(",").join("").trim();
          }
          var image_url = $s('.proImg img').attr("src");
...
var parameters =  encodeURIComponent(prod) + "~*~*" + myPrice + "~*~*" + encodeURIComponent(image2) + "~*~*" + encodeURIComponent(url) + "~*~*429" ;
...
button.addEventListener("click", function(){
  ...
  document.getElementById('bhWidget').innerHTML = '<div class="line fk-font-12" style="margin-bottom: 4px;">Thank You ! We appreciate your motive to save energy :)</div>';
      ...
      if($s('.product-options-bottom').length>0){
      $s('.product-options-bottom').before(strToDisp);
      }
...
if(flagToDisp==0){
  var imgURL2 = chrome.extension.getURL("watch-price1.png");
if($s('.product-options-bottom').length>0){
$s('.product-options-bottom').before('<div id="bhWidget"><a id="addWatchList" style="margin-top: 4px;" alt="Add to Watch List" title="Add to BuyHatke Watch List and get notifications on price drop" href="javascript:void();" class="fk-inline-block buy-btn fksk-buy-btn"><img style="margin-left:-12px;" src=' + imgURL2 +'></a></div>');
 }
 else if($s('#size-selection').length>0){
...
else if($s('.product-options-bottom').length>0){
  $s('.product-options-bottom').before(strToDisp);
}
else if($s('#size-selection').length>0){
  $s('#size-selection').after(strToDisp);
...
$s('#addtocartbtn').after('<a target="_blank" href="' + url + '"><input type="button" id="addtocartbtn" class="cartBtnGreen" title="Compare via Buyhatke"  value="Compare Now"></a>');

var price = 0;
if($s('.mrp').find('.fbold').text().trim().split("MRP").length>1){
price = $s('.mrp').find('.fbold').text().trim().split("MRP")[1].trim();
}
else {
price = $s('.mrp').find('.fbold').text().trim().split(",").join("");
}
