...
var parameters = "ext_id=" + ext_id + "&auth_val=" + ext_auth + "&pid=" + pid + "&web=zovi";
...
$s(function () {
  //////console.log(dataString);
    $s('#container2').highcharts({
      ...
      title: {
        text: 'Should I purchase ' + $s('#product-title').text().trim() + " now ?"
      },
      subtitle: {
        text: 'Price Variance of ' + $s('#product-title').text().trim() + " at Zovi"
      },
      xAxis: {
        type: 'datetime',
...
function postProcessor(coupon, i){
  checkPass = 1;
  var savingsText = $s('.applied-coupon').text();
    if(savingsText!=""){
...
function preProcessor(i, coupon){
  ...
  $s('.coupon_code').find('input').val(coupon);
  $s('.coupon').find('.submit').find('button').click();
...
var parameters = "ext_id=" + ext_id + "&auth_val=" + ext_auth + "&pos=22";
httpq4.open("POST", myurl, true);
httpq4.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
httpq4.onreadystatechange = function(){
if (httpq4.readyState == 4) {
...
if(curURL.split('zovi.com/buy').length>1){
  ...
  console.log("TEst passed " + $s('.coupon').length);
  if($s('.coupon').length>0){
  ...
  //$s('#bagoffer').before("<a id='couponClick' href='javascript:void();'><img style='margin-left:65px;' src='" + imgURL + "'></a>");
  addToDOM();
...
function reportPurchase(){
  var curURL = window.location.href;
  if(curURL.split('zovi.com/buy').length>1){
    chrome.runtime.sendMessage({processDONE: "Zovi"}, function(response) {
    });
  }
}
...
var myurl = "http://compare.buyhatke.com/addWatchList.php";
 var name = $s('meta[property="og:title"]').attr('content');
          if($s(".old-price").length > 0)
          {
            var price = $s('body').html().split("discounted_price")[1].split(",")[0].split(":")[1];
          }
          else if($s("#price").text().split("Rs").length > 1)
          {
            var price = $s("#price").text().split("Rs")[1].split(",").join("").trim();
          }
          else
          {
            var price = $s("#price").text().split(",").join("").trim();
          }
          var image_url = $s("#detail-image img").attr("src");
...
var parameters =  encodeURIComponent(prod) + "~*~*" + myPrice + "~*~*" + encodeURIComponent(image2) + "~*~*" + encodeURIComponent(url) + "~*~*425" ;
...
button.addEventListener("click", function(){
  ...
  document.getElementById('bhWidget').innerHTML = '<div class="line fk-font-12" style="margin-bottom: 4px;">Thank You ! We appreciate your motive to save energy :)</div>';
      ...
      if($s('#thumbs').length>0){
      $s('#thumbs').before(strToDisp);
      }
...
if(flagToDisp==0){
  var imgURL2 = chrome.extension.getURL("watch-price1.png");
if($s('#thumbs').length>0){
$s('#thumbs').before('<div id="bhWidget"><a id="addWatchList" style="margin-top: 4px;" alt="Add to Watch List" title="Add to BuyHatke Watch List and get notifications on price drop" href="javascript:void();" class="fk-inline-block buy-btn fksk-buy-btn"><img style="margin-left:-12px;" src=' + imgURL2 +'></a></div>');
 }
 else if($s('#size-selection').length>0){
...
var name = $s('#product-title').text().trim();
origProd = name;
name = name.split("(")[0];
...
price = $s('#price-block').text().split("Rs")[2].split(" ")[0];
...
if(parseInt(results[indexSelected].price)<=parseInt(origPrice)&&notFound==0&&document.getElementById('detailOutWrap') == null) {
   ...
   $s('#nav-header').css("margin-top", "0px");
