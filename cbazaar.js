...
$s = jQuery.noConflict();
var name = $s('.detailHead').find('h1').text().trim();
name2 = name.split(" ");
newName = "";

for(k=1;k<name2.length;k++){
  newName = newName + name2[k] + " ";
}
name = newName.trim();
origProd = name;
name = name.split("(")[0];
...
var priceCheck = $s('.offprice').length;
if(priceCheck!=0){
  price = $s('.offprice').text().split("Rs.")[1];
}
else {
  price = $s('#h5Amount').text().split("Rs.")[1];
}
