...
var name = $s('#product-name1').text().trim();
                if($s("#old_price_display").length > 0)
                {
                  var price = $s('#our_price_display').text().split("Rs.")[1].split(",").join("").trim();
                }
                else if($s('.price .our_price_display').text().split("Rs.").length > 1)
                {
                  var price = $s('.price .our_price_display').text().split("Rs.")[1].split(",").join("").trim();
                }
                else if($s('.price_block').length>0){
                  var price = $s('.price_block').find('h1').text().split("Rs.")[1].split(",").join("").trim();
                }
                else if($s('.price-box').length>0){
                  var price = $s('.price-box').text().split("INR")[1].trim().split(",").join("");
                }
                else
                {
                  var price = $s('.price .our_price_display').text().split(",").join("").trim();

                }
                var image_url = $s('#ul-moreviews').find('img:eq(0)').attr("src");
...
var parameters =  encodeURIComponent(prod) + "~*~*" + myPrice + "~*~*" + encodeURIComponent(image2) + "~*~*" + encodeURIComponent(url) + "~*~*901" ;
...
button.addEventListener("click", function(){
  ...
  document.getElementById('bhWidget').innerHTML = '<div class="line fk-font-12" style="margin-bottom: 4px;">Thank You ! We appreciate your motive to save energy :)</div>';
      ...
      if($s('.select_size').length>0){
      $s('.select_size').before(strToDisp);
      }
...
if(flagToDisp==0){
  var imgURL2 = chrome.extension.getURL("watch-price1.png");
if($s('.select_size').length>0){
$s('.select_size').before('<div id="bhWidget" style="margin-left:80px;"><a id="addWatchList" style="margin-top: 4px;" alt="Add to Watch List" title="Add to BuyHatke Watch List and get notifications on price drop" href="javascript:void();" class="fk-inline-block buy-btn fksk-buy-btn"><img style="margin-left:-12px;" src=' + imgURL2 +'></a></div>');
 }
 else if($s('.price-box').length>0 && $s('.price-box').length == 1){
  $s('.price-box').after('<div id="bhWidget" style="margin-left:80px;"><a id="addWatchList" style="margin-top: 4px;" alt="Add to Watch List" title="Add to BuyHatke Watch List and get notifications on price drop" href="javascript:void();" class="fk-inline-block buy-btn fksk-buy-btn"><img style="margin-left:-12px;" src=' + imgURL2 +'></a></div>');
 }
...
else if($s('.select_size').length>0){
  $s('.select_size').before(strToDisp);
}
else if($s('.price-box').length>0 && $s('.price-box').length == 1){
  $s('.price-box').after(strToDisp);
