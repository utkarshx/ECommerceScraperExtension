...
function reportPurchase(){
  ...
  if(curURL.split('babyoye.com/control').length>1){
    chrome.runtime.sendMessage({processDONE: "Babyoye"}, function(response) {
    });
  }
}
...
$s = jQuery.noConflict();
var name = $s('.productName').text().trim();
origProd = name;
...
$s('#adcart').after('<a target="_blank" href=' + url + '><button id="ad2-cart-but" class="button white_grey addCart" type="button" style="width: 200px!important;" >Compare</button></a>');

var price = $s('#current_product_price').text().split(",").join("").trim();
