...
function postProcessor(coupon, i){
  if($('.ui-dialog').css("display")=="block"){
    ...
    ind_req2 = -1;
    for(k=0;k<totLen;k++){
      if($('.fare_row:eq(' + k + ')').text().split("Discount").length>1){
        ind_req2 = k;
      }
    }

    if(ind_req2!=-1){
      var msg = $('.fare_row:eq(' + ind_req2 + ')').text().trim();
      savings = msg.split("Promo Discount");
      if(savings.length>1){
        savings = savings[1];
        ...
        savings = savings[0];
        console.log("Actual savings is " + savings);
...
var parameters = "ext_id=" + ext_id + "&auth_val=" + ext_auth + "&pos=12";
