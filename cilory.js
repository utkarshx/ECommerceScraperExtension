...
var myurl = "http://compare.buyhatke.com/addWatchList.php";
 var name = $s('[itemprop="name"]').text();
          var price = $s('[itemprop="price"]').text().split(",").join("").trim();
          var image_url = $s('[itemprop="image"]').attr("src");
...
var parameters =  encodeURIComponent(prod) + "~*~*" + myPrice + "~*~*" + encodeURIComponent(image2) + "~*~*" + encodeURIComponent(url) + "~*~*469" ;
...
button.addEventListener("click", function(){
  ...
  document.getElementById('bhWidget').innerHTML = '<div class="line fk-font-12" style="margin-bottom: 4px;">Thank You ! We appreciate your motive to save energy :)</div>';
      ...
      if($s('.product_attributes').length>0){
      $s('.product_attributes').before(strToDisp);
      }
      else if($s('.content_prices').length>0){
        $s('.content_prices:eq(0)').after(strToDisp);
      }
      var button = document.getElementById("removeMe2");
button.addEventListener("click", function(){
  removeAlert();
...
if(flagToDisp==0){
  var imgURL2 = chrome.extension.getURL("watch-price1.png");
if($s('.product_attributes').length>0){
$s('.product_attributes').before('<div id="bhWidget"><a id="addWatchList" style="margin-top: 4px;" alt="Add to Watch List" title="Add to BuyHatke Watch List and get notifications on price drop" href="javascript:void();" class="fk-inline-block buy-btn fksk-buy-btn"><img style="margin-left:-12px;" src=' + imgURL2 +'></a></div>');
 }
 else if($s('.content_prices').length>0){
  $s('.content_prices:eq(0)').after('<div id="bhWidget"><a id="addWatchList" style="margin-top: 4px;" alt="Add to Watch List" title="Add to BuyHatke Watch List and get notifications on price drop" href="javascript:void();" class="fk-inline-block buy-btn fksk-buy-btn"><img style="margin-left:-12px;" src=' + imgURL2 +'></a></div>');
 }
...
else if($s('.product_attributes').length>0){
  $s('.product_attributes').before(strToDisp);
}
else if($s('.content_prices').length>0){
  $s('.content_prices:eq(0)').after(strToDisp);
...
var name = $s('#ajax_product_name').text().trim();
origProd = name;
name = name.split("(")[0];
...
$s('#add_to_cart').find('input').after('<a title="Compare via Compare Hatke" target="_blank" href=' + url + '><input id="buyhatkeInput" rel="nofollow" style="max-width:80px;" value="Compare" class=" exclusive button orange newbtn"></a>');

var button = document.getElementById("buyhatkeInput");
button.addEventListener("click", function() {
  window.location.href = $s('#add_to_cart').find('a').attr("href");
}, false);

var final2send = url.split("products/");
var msgToSend = final2send[1] + "~*~*" + $s('#our_price_display').text().trim();
