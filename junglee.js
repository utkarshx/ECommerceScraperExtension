...
var name = $('.mainContent').find('h1:eq(0)').text().trim();
name = name.split("(")[0];
origProd = name;
...
$('.offer-price-info:eq(0)').after('<a href="' + url + '" class="visitbutton button larger-button" target="_blank" title="Compare via Buyhatke"><span class="buttonfontstyle">Compare via Buyhatke </span><span class="orangechev"></span></a>');

// $('.action_bar').find('a:eq(1)').after('<a title="Compare via Compare Hatke" target="_blank" id="priceCompare" class="various button_new big left" href="'+ url +'">Compare price</a>');

price = $('.whole-price:eq(0)').text().trim().split(",").join("");
...
if($('#mainTop-1').text().trim().split("Jewellery").length >=2 || $('#mainTop-1').text().trim().split("Clothing").length >=2 || $('#mainTop-1').text().trim().split("Watches").length >=2 || $('#mainTop-1').text().trim().split("Shoes & Accessories").length >=2){
  isApparels = true;
