...
function reportPurchase(){
  ...
  if(curURL.split('shopping.indiatimes.com/control').length>1){
    chrome.runtime.sendMessage({processDONE: "IndiaTimes"}, function(response) {
    });
  }
}
...
function preProcessor(i, coupon){
  ...
  document.getElementsByClassName('mergecoupon')[0].click();
...
var parameters = "ext_id=" + ext_id + "&auth_val=" + ext_auth + "&pos=8";
...
function checkSavings(){
  ...
  var totLen = $('.subtotal_hd').length;
  for(i=0;i<totLen;i++){
    var curHead = $('.subtotal_hd:eq(' + i + ')').text();
    if(curHead.split("Coupon").length>1){
      var curValue = $('.subtotal_hd:eq(' + i + ')').nextUntil('span').text();
  ...
  if(savings>0){
    varName = "savingsIndia" + getCookie("doneTill");
...
if(curURL.split('indiatimes.com/control').length>1){
...
var myurl = "http://compare.buyhatke.com/addWatchList.php";
          ...
          if($('[itemprop="price"]').length > 0){
            ...
            {
              var price = $('[itemprop="price"]').text().split("`")[1].split(',').join("").trim();
            }
            else
            {
          ...
          else if($(".offerprice").length > 0)
          {
            if($('[itemprop="price"]').text().split("`").length > 1)
            {
              var price = $(".offerprice").text().split("`")[1].split(',').join("").trim();
            }
            else
            {
              var price = $(".offerprice").text().split("`")[1].split(',').join("").trim();
          ...
          var image_url = $("#zoom1 img").attr("src");
...
var parameters =  encodeURIComponent(prod) + "~*~*" + myPrice + "~*~*" + encodeURIComponent(image2) + "~*~*" + encodeURIComponent(url) + "~*~*401" ;
...
var name = $('.producthead').find('h1').text().trim();
if(name==""){
  name = $('h1').text();
}
name = name.split("(")[0];
...
$('.buynowpdp').after('<a target="_blank" href=' + url + ' title="Compare via Compare Hatke"><img src='+ imgURL +' alt="Compare Now"></a>');

var final2send = url.split("products/");
var msgToSend = final2send[1] + "~*~*" + $('.pricetab').find('.offerprice').text().trim().split(",").join("").split("`")[1].trim();
...
$(document).ready(function(){
  ...
  var origPrice = $('.pricetab').find('.offerprice').text().trim().split(",").join("").split("`")[1].trim();
