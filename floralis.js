...
var name = $('.descarea').find('h2').text();
name = name.split("[")[0];
origProd = name;
...
var imgURL = chrome.extension.getURL("floralis.png");

$('.chkbg').find('li:eq(2)').after('<li><img src='+ imgURL + ' title="Compare via Compare Hatke" align="absmiddle"><a target="_blank" href=' + url + '>Compare</a></li>');

var final2send = url.split("products/");
var msgToSend = final2send[1] + "~*~*" + $('.pricerate').find('span:eq(0)').text().split("Rs.")[1].split(",").join("").trim();
...
chrome.runtime.onMessage.addListener(
 function(request, sender) {
  ...
  var origPrice = $('.pricerate').find('span:eq(0)').text().split("Rs.")[1].split(",").join("").trim();
