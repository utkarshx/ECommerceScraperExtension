...
var name = $s('.product-title-text:eq(0)').text().trim();
               if($s('.product-price-text:eq(0)').text().split("Rs.").length > 1)
               {
                var price = $s('.product-price-text:eq(0)').text().split("Rs.")[1].split(",").join("").trim();
              }
              else
              {
                var price = $s('.product-price-text:eq(0)').text().split(",").join("").trim();

              }
              var image_url = $s('.prod_img img').attr('src');
...
var parameters =  encodeURIComponent(prod) + "~*~*" + myPrice + "~*~*" + encodeURIComponent(image2) + "~*~*" + encodeURIComponent(url) + "~*~*902" ;
...
button.addEventListener("click", function(){
  ...
  document.getElementById('bhWidget').innerHTML = '<div class="line fk-font-12" style="margin-bottom: 4px;">Thank You ! We appreciate your motive to save energy :)</div>';
      ...
      if($s('.product-title-text-text').length>0){
      $s('.product-title-text:eq(1)').after(strToDisp);
      }
...
if(flagToDisp==0){
  var imgURL2 = chrome.extension.getURL("watch-price1.png");
if($s('.product-title-text').length>0){
$s('.product-title-text:eq(1)').after('<div id="bhWidget"><a id="addWatchList" style="margin-top: 4px;" alt="Add to Watch List" title="Add to BuyHatke Watch List and get notifications on price drop" href="javascript:void();" class="fk-inline-block buy-btn fksk-buy-btn"><img style="margin-left:-12px;" src=' + imgURL2 +'></a></div>');
 }
 else if($s('.price-box').length>0 && $s('.price-box').length == 1){
...
else if($s('.product-title-text').length>0){
  $s('.product-title-text:eq(1)').after(strToDisp);
}
else if($s('.price-box').length>0 && $s('.price-box').length == 1){
  $s('.price-box').after(strToDisp);
