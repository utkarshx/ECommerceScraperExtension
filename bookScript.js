...
var title = $('.prdcol2').find('h1').text().trim();
origProd = title;
title = title.split(" ");
var title2 = title.join("-");
var urlToFollow = url + title2;
var imgURL = chrome.extension.getURL("bookadda.png");
...
var msgToSend = final2send[1] + "~*~*" + parseFloat($('.actlprc').text().split("Rs.")[1].trim());
...
chrome.runtime.onMessage.addListener(
 function(request, sender) {
  ...
  var origPrice = parseFloat($('.actlprc').text().split("Rs.")[1].trim());
