...
var prod = $('.pDesc').find('h1').text().trim();
...
var image = "http://www.cromaretail.com/" + $('.productImage').find('img:eq(0)').attr("src");
}
else {
  var image = "";
}
var parameters =  encodeURIComponent(prod) + "~*~*" + myPrice + "~*~*" + encodeURIComponent(image) + "~*~*" + encodeURIComponent(url) + "~*~*10" ;
...
var name = $('.pDesc').find('h1').text().trim();
name = name.split("(")[0];
origProd = name;
...
if($('#buyNow').length>0){
  if(flagToDisp==0){
$('#buyNow').before('<div id="bhWidget"><a id="addWatchList" style="margin-top: 10px;margin-left:-10px;" alt="Add to Watch List" title="Add to BuyHatke Watch List and get notifications on price drop" href="javascript:void();" class="fk-inline-block buy-btn fksk-buy-btn"><img src=' + imgURL2 +'></a></div>');
...
$('#buyNow').after('<div id="btnBuyNowC2"><a target="_blank" href="' + url + '" id="btnBuyNow2" class="btn " style="background-color: transparent;"><img src="' + imgURL + '"></a></div>');
...
if(parseInt(results[indexSelected].price)<=parseInt(origPrice)&&notFound==0&&document.getElementById('detailOutWrap') == null) {
   ...
   $('.navigation-all').css("margin-top", "40px");
  $("#dd_menu_list").hover(
  function() {
    msg2 = "Results Viewed";
