...
var imgURL = chrome.extension.getURL("indiacakes.png");

$('.button-submit-action:eq(1)').after('<a target="_blank" href=' + url + '><img src=' + imgURL +' title="Compare via Compare Hatke"></a>');

var final2send = url.split("products/");
var msgToSend = final2send[1] + "~*~*" + $('.price-update:eq(0)').text().split("|")[0].trim().split("Rs.")[1].split(",").join("");
...
chrome.runtime.onMessage.addListener(
 function(request, sender) {
  ...
  var origPrice = $('.price-update:eq(0)').text().split("|")[0].trim().split("Rs.")[1].split(",").join("");
