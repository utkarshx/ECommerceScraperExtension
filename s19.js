...
var myurl = "http://compare.buyhatke.com/addWatchList.php";
 var name = $s('meta[property="og:title"]').attr('content');
          if($s('.price').text().split("/-").length > 1)
          {
            var price = $s('.price').text().split("/-")[0].split(",").join("").trim();
          }
          else
          {
...
var parameters =  encodeURIComponent(prod) + "~*~*" + myPrice + "~*~*" + encodeURIComponent(image2) + "~*~*" + encodeURIComponent(url) + "~*~*422" ;
...
button.addEventListener("click", function(){
  ...
  document.getElementById('bhWidget').innerHTML = '<div class="line fk-font-12" style="margin-bottom: 4px;">Thank You ! We appreciate your motive to save energy :)</div>';
      ...
      if($s('#product-options-wrapper').length>0){
      $s('#product-options-wrapper').before(strToDisp);
      }
...
if(flagToDisp==0){
  var imgURL2 = chrome.extension.getURL("watch-price1.png");
if($s('#product-options-wrapper').length>0){
$s('#product-options-wrapper').before('<div id="bhWidget"><a id="addWatchList" style="margin-top: 4px;" alt="Add to Watch List" title="Add to BuyHatke Watch List and get notifications on price drop" href="javascript:void();" class="fk-inline-block buy-btn fksk-buy-btn"><img style="margin-left:-12px;" src=' + imgURL2 +'></a></div>');
 }
 else if($s('#size-selection').length>0){
...
else if($s('#product-options-wrapper').length>0){
  $s('#product-options-wrapper').before(strToDisp);
}
else if($s('#size-selection').length>0){
  $s('#size-selection').after(strToDisp);
...
var name = $s('.product-name').find('h1').text();
origProd = name;
name = name.split("(")[0];
...
var priceCheck = $s('.special-price').length;

if(priceCheck != 0){
  price = $s('.special-price').text().trim().split("/-")[0].trim();
}
else {
  price = $s('.regular-price').text().trim().split("/-")[0].trim();
}
