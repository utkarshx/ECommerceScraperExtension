...
function postProcessor(coupon, i){
  if($('#voucher-error').hasClass("hide") || $('#voucher-success').hasClass("hide")){
    if($('#voucher-error').hasClass("hide")){
      if($('.cart-line-voucher').length>0){
        savings = $('.cart-line-voucher').text().trim().split("Rs.")[1];
        savings = parseFloat(savings);
      }
    }
    else if($('#voucher-success').hasClass("hide")){
      savings = 0;
    }
...
function preProcessor(i, coupon){
  ...
  $('#voucher-code').val(coupon);
  document.getElementById('submit-voucher').click();
...
var parameters = "ext_id=" + ext_id + "&auth_val=" + ext_auth + "&pos=15";
...
if(curURL.split('foodpanda.in/checkout').length>1){
  var imgURL = chrome.extension.getURL("apply-coupon.png");
  console.log("TEst passed");
  if($('#checkout-voucher').length>0){
  $('#checkout-voucher').after("<a id='couponClick' href='javascript:void();'><img style='margin-top:15px;margin-left:20px;' src='" + imgURL + "'></a>");
  addToDOM();
