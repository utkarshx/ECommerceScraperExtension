...
function reportPurchase(){
  ...
  if(curURL.split('secure.shopclues.com').length>1){
    chrome.runtime.sendMessage({processDONE: "ShopClues"}, function(response) {
    });
  }
}
...
var parameters = "ext_id=" + ext_id + "&auth_val=" + ext_auth + "&pid=" + pid + "&web=shopclues";
...
$(function () {
  //////console.log(dataString);
    $('#container2').highcharts({
      ...
      title: {
        text: 'Should I purchase ' + $('.product-info').find('h1').text().trim() + " now ?"
      },
      subtitle: {
        text: 'Price Variance of ' + $('.product-info').find('h1').text().trim() + " at ShopClues"
      },
      xAxis: {
        type: 'datetime',
...
var parameters = "ext_id=" + ext_id + "&auth_val=" + ext_auth + "&pos=7";
httpq4.open("POST", myurl, true);
httpq4.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
httpq4.onreadystatechange = function(){
if (httpq4.readyState == 4) {
...
if(curURL.split('dispatch=checkout.checkout').length>1 && ($('#coupon_field').length>0)){
  ...
  if($('.form_twocolumnwithbutton_row').length>0){
  $('.form_twocolumnwithbutton_row:eq(0)').after("<a id='couponClick' href='javascript:void();'><img style='margin-left:65px;' src='" + imgURL + "'></a>");
  addToDOM();
...
function sendPairs(){
  arrayToSend = [];
  var slider = $('.bs_cntnr_item_vr');
  var sliderLength = slider.length;
  for(i=0;i<sliderLength;i++){
    var url = $('.bs_cntnr_item_vr:eq(' + i + ')').find('a:eq(0)').attr("href");
    if(url!= undefined){
      url2 = url.split("?");
      PID = url2[0];
      var price = $('.bs_cntnr_item_vr:eq(' + i + ')').find('.bs_main_price_vr');
      if(price.length>0){
        price = price.text();
  ...
  var sliderLength = slider.length;
  for(i=0;i<sliderLength;i++){
    var url = $('.bs_cntnr_item:eq(' + i + ')').find('a:eq(0)').attr("href");
    if(url!= undefined){
      url2 = url.split("?");
      PID = url2[0];
      var price = $('.bs_cntnr_item:eq(' + i + ')').find('.bs_main_price');
      if(price.length>0){
        price = price.text();
        price = price.split("Rs.");
        price = price[1];
  ...
  var slider = $('.box_GridProduct');
  var sliderLength = slider.length;
  for(i=0;i<sliderLength;i++){
    var url = $('.box_GridProduct:eq(' + i + ')').find('a:eq(0)').attr("href");
    if(url!= undefined){
      url2 = url.split("?");
      PID = url2[0];
      var price = $('.box_GridProduct:eq(' + i + ')').find('.box_metacategory_priceoffer');
      if(price.length==0){
       var price = $('.box_GridProduct:eq(' + i + ')').find('.box_GridProduct_priceoffer');
      }
      if(price.length>0){
        price = price.text();
        price = price.split("Rs.");
  ...
  chrome.runtime.sendMessage({pairsShopClues: arrayToSend}, function(response) {
    });
}
...
var len = $('.product-info').find('.price').length;
var price = $('.price:eq(' + (len-1) + ')').text();
price = price.split("Rs.");
if(price.length>1){
  price = price[1];
}
...
var prod = $('.product-info').find('h1').text().trim();
var cur_url = window.location.href;
if($('.image-border').length>0){
var image2 = $('.image-border').find('img').attr("src");
}
else {
  var image2 = "";
}
...
image = image2;
    ...
    //console.log(curData);
    chrome.runtime.sendMessage({curDataShop: curData}, function(response) {
    });
  }
}
...
price = parseFloat(price);
  for(i=0;i<response.farewell.length;i++){
   ...
   if(filterURL2==filterURL || filterURL2 == filterURL + "?" || filterURL2 + "?" == filterURL){
    ...
    if(price!=0){
      response.farewell[i].cur_price = price;
    }
...
button.addEventListener("click", function(){
  ...
  document.getElementById('bhWidget').innerHTML = '<div class="line fk-font-12" style="margin-bottom: 4px;">Thank You ! We appreciate your motive to save energy :)</div>';
      ...
      $('.button-submit-action:eq(0)').before(strToDisp);
      var button = document.getElementById("removeMe2");
button.addEventListener("click", function(){
  removeAlert();
  document.getElementById('bhWidget').innerHTML = '<div class="line fk-font-12" style="margin-bottom: 4px;">Thank You ! We appreciate your motive to save energy :)</div>';
...
var parameters =  encodeURIComponent(prod) + "~*~*" + myPrice + "~*~*" + encodeURIComponent(image2) + "~*~*" + encodeURIComponent(url) + "~*~*4" ;
...
var name = $('.product-info').find('h1').text().trim();
name = name.split("(")[0];
origProd = name;
...
$('.button-submit-action:eq(0)').after('<a target="_blank" href="'+ url +'"><span class="button-submit-action myClass .pro_det_add_to_cart_butto"><input type="button" value="Compare"></span></a>');

if(flagToDisp==0){
$('.button-submit-action:eq(0)').before('<div id="bhWidget"><a id="addWatchList" style="margin-top: 4px;" alt="Add to Watch List" title="Add to BuyHatke Watch List and get notifications on price drop" href="javascript:void();" class="fk-inline-block buy-btn fksk-buy-btn"><img style="margin-left:-12px;" src=' + imgURL2 +'></a></div>');
}
else {
  $('.button-submit-action:eq(0)').before(strToDisp);
}
...
//var price = parseFloat($('.price-update').text().trim().split("Rs.")[1].split(",").join(""));

if($('.box_specialoffer_message_text_price').length!=0){
	price = $('.box_specialoffer_message_text_price').text().split("Rs.")[1].trim().split("/")[0].split(",").join("");
}


var final2send = url.split("products/");
var msgToSend = final2send[1] + "~*~*" + parseFloat($('.price-update').text().trim().split("Rs.")[1].split(",").join(""));

if($('.breadcrumbs').text().trim().split('Jewelry').length>1||$('.breadcrumbs').text().trim().split('Watch').length>1||$('.breadcrumbs').text().trim().split('Fashion').length>1||$('.breadcrumbs').text().trim().split('Footwear').length>1||$('.breadcrumbs').text().trim().split('Jewelry').length>1){
