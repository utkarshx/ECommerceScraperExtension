...
function reportPurchase(){
  ...
  if(curURL.split('pepperfry.com/checkout').length>1){
    chrome.runtime.sendMessage({processDONE: "Pepperfry"}, function(response) {
    });
  }
}
...
var parameters =  encodeURIComponent(prod) + "~*~*" + myPrice + "~*~*" + encodeURIComponent(image2) + "~*~*" + encodeURIComponent(url) + "~*~*333" ;
...
button.addEventListener("click", function(){
  ...
  document.getElementById('bhWidget').innerHTML = '<div class="line fk-font-12" style="margin-bottom: 4px;">Thank You ! We appreciate your motive to save energy :)</div>';
      ...
      if($('.pdpbuynottab').length>0){
      $('.pdpbuynottab').before(strToDisp);
      }
      else if($('.vip_price_block_2').length>0){
        $('.vip_price_block_2').after(strToDisp);
      }
      var button = document.getElementById("removeMe2");
button.addEventListener("click", function(){
  removeAlert();
...
if(flagToDisp==0){
  var imgURL2 = chrome.extension.getURL("watch-price1.png");
if($('.pdpbuynottab').length>0){
$('.pdpbuynottab').before('<div id="bhWidget"><a id="addWatchList" style="margin-top: 4px;" alt="Add to Watch List" title="Add to BuyHatke Watch List and get notifications on price drop" href="javascript:void();" class="fk-inline-block buy-btn fksk-buy-btn"><img style="margin-left:-12px;" src=' + imgURL2 +'></a></div>');
 }
 else if($('.vip_price_block_2').length>0){
  $('.vip_price_block_2').after('<div id="bhWidget"><a id="addWatchList" style="margin-top: 4px;" alt="Add to Watch List" title="Add to BuyHatke Watch List and get notifications on price drop" href="javascript:void();" class="fk-inline-block buy-btn fksk-buy-btn"><img style="margin-left:-12px;" src=' + imgURL2 +'></a></div>');
 }
...
if(name==""){
  var name = $('h1').text().trim();
          ...
          var image_url = $('meta[property="og:image"]').attr('content').trim();
origProd = name;
var check = $(document).find('.special-price');
...
url = urlSplit.join("");
$('.vip_button_block_2 input:eq(0)').after('<a target="_blank" href="' + url + '"><input type="button" class="button_large" value="Compare Now"></a>');
 if($('[itemprop="price"]').text().split("Rs.").length > 1)
          {
            var price = $('[itemprop="price"]').text().split("Rs.")[1].split(",").join("").trim();
          }
...
if(parseInt(results[indexSelected].price)<=parseInt(origPrice)&&notFound==0&&document.getElementById('detailOutWrap') == null) {
   ...
   $('body').before('<div id="detailOutWrap"><div id="detailInWrap"><a target="_blank" href="http://compare.buyhatke.com" title="Visit Buyhatke"><img id="details_logo" src="http://compare.buyhatke.com/images/logo_small.png"></a><div id="details">Hurray !  Massive savings found. This product is available for <span id="detail_cost"><img src="http://compare.buyhatke.com/images/rupeeK.png"> ' + results[indexSelected].price + '</span> at <span id="detail_store">' + siteName2 + '</span><a style="display:inline!important;" href="' + results[indexSelected].link + '" target="_blank"><input type="button" value=" BUY IT NOW" style="display: inline;width: 9%!important;" ></a>or<div class="drop_down" id="compare_now" onmouseover="cancel=true;">COMPARE PRICES<div class="drop_down_symbol"></div><div id="dd_menu"><head><div id="dd_menu_header">Showing <span>' + results.length + '</span> results</div></head>' + string + string2);
...
blinker();
 	 ...
 	 $('body').before('<div id="detailOutWrap"><div id="detailInWrap"><div id="details">Hurray !  Massive savings found. Click to know more <a href="' + url + '" target="_blank"><input type="button" value=" COMPARE PRICES" style="display: inline;width: 9%!important;"></a><a target="_blank" href="https://www.facebook.com/sharer/sharer.php?u=http%3A%2F%2Fextension.buyhatke.com" id="notify_fshare"></a></div><a href="#" id="detailClose" >x</a></div></div>');
  $('body').css("margin-top", "45px");
