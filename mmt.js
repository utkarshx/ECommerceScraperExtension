...
function postProcessor(coupon, i){
  if($('.dialogue').length==1){
    var msg = $('.dialogue').text();
    if(msg.split("Success").length>1){
      savings = msg.split("Rs.");
      if(savings.length>1){
        savings = savings[1];
    ...
    document.getElementById('ok').getElementsByTagName("a")[0].click();
...
function preProcessor(i, coupon){
  $('#ecouponId').val(coupon);
  document.getElementById('ecouponBtn').click();
