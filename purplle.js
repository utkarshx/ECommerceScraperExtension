...
var name = $('meta[property="og:title"]').attr('content');
              ...
              if($('.normal-price').text().split("Rs.").length > 1)
              {
                var price = $('.normal-price:eq(0)').text().split("Rs.")[1].split(",").join("").trim();
              }
              else
              {
                var price = $('.normal-price:eq(0)').text().split(",").join("").trim();
              }
...
var parameters =  encodeURIComponent(prod) + "~*~*" + myPrice + "~*~*" + encodeURIComponent(image2) + "~*~*" + encodeURIComponent(url) + "~*~*900" ;
...
chrome.runtime.sendMessage({detailArray: "haiKya"}, function(response) {
   ...
   filterURL = filterURL.split("?")[0];
              if($('.normal-price').text().split("Rs.").length > 1)
              {
                var price = $('.normal-price').text().split("Rs.")[1].split(",").join("").trim();
              }
              else
              {
                var price = $('.normal-price').text().split(",").join("").trim();
              }
...
button.addEventListener("click", function(){
  ...
  document.getElementById('bhWidget').innerHTML = '<div class="line fk-font-12" style="margin-bottom: 4px;">Thank You ! We appreciate your motive to save energy :)</div>';
      ...
      if($('.rate-share').length>0){
      $('.rate-share:eq(0)').before(strToDisp);
      }
...
if(flagToDisp==0){
  var imgURL2 = chrome.extension.getURL("watch-price1.png");
if($('.rate-share').length>0){
$('.rate-share:eq(0)').before('<div id="bhWidget"><a id="addWatchList" style="margin-top: 4px;" alt="Add to Watch List" title="Add to BuyHatke Watch List and get notifications on price drop" href="javascript:void();" class="fk-inline-block buy-btn fksk-buy-btn"><img style="margin-left:-12px;" src=' + imgURL2 +'></a></div>');
 }
 else if($('.price-box').length>0 && $('.price-box').length == 1){
