...
var name = $('.productName').find('h1').text();
origProd = name;
...
url = urlSplit.join("");
var imgURL = chrome.extension.getURL("FNP.png");
$('.actionButton3').find('span:eq(0)').after('<a target="_blank" href="' + url + '"><span style="margin-left:-5%;margin-top:-1%"><img src="' + imgURL + '"></span></a>');


var final2send = url.split("products/");
var msgToSend = final2send[1] + "~*~*" + parseFloat($('.offerPriceDiv').find('span:eq(2)').text().trim());
...
chrome.runtime.onMessage.addListener(
 function(request, sender) {
  ...
  var origPrice = parseFloat($('.offerPriceDiv').find('span:eq(2)').text().trim());
