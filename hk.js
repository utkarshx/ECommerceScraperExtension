...
var myurl = "http://compare.buyhatke.com/addWatchList.php";
          ...
          {
            var price = $('.sucss-txt').text().split("Rs.")[1].split(",").join("").trim();
          }
          else
          {
            var price = $('.sucss-txt').text().split(",").join("").trim();
          }

          var image_url = $('.img-box img').attr("src");
...
var parameters =  encodeURIComponent(prod) + "~*~*" + myPrice + "~*~*" + encodeURIComponent(image2) + "~*~*" + encodeURIComponent(url) + "~*~*921" ;
...
button.addEventListener("click", function(){
  ...
  document.getElementById('bhWidget').innerHTML = '<div class="line fk-font-12" style="margin-bottom: 4px;">Thank You ! We appreciate your motive to save energy :)</div>';
      ...
      if($('.variant-diff-attr-div').length>0){
      $('.variant-diff-attr-div').after(strToDisp);
      }
...
if(flagToDisp==0){
  var imgURL2 = chrome.extension.getURL("watch-price1.png");
if($('.variant-diff-attr-div').length>0){
$('.variant-diff-attr-div').after('<div id="bhWidget"><a id="addWatchList" style="margin-top: 4px;" alt="Add to Watch List" title="Add to BuyHatke Watch List and get notifications on price drop" href="javascript:void();" class="fk-inline-block buy-btn fksk-buy-btn"><img style="margin-left:-12px;" src=' + imgURL2 +'></a></div>');
 }
 else if($('.more-less').length>0){
...
else if($('.variant-diff-attr-div').length>0){
  $('.variant-diff-attr-div').after(strToDisp);
}
else if($('.more-less').length>0){
  $('.more-less').after(strToDisp);
...
price = $('.sucss-txt').text().split("Rs.")[1].trim();
price = price.split(",").join("");
var url = "http://compare.buyhatke.com/products/" + name;
 var imgURL = chrome.extension.getURL("healthkart2.png");
 if($('.addToCartButton').length==1){
$('.addToCartButton').after('<a style="margin-left: -45%" target="_blank" href=' + url + ' ><img src=' + imgURL +' title="Compare via Compare Hatke" style="margin-left: 162px;"></a>');
}
else if($('.addToCartButton').length>1){
	 var imgURL = chrome.extension.getURL("healthkart.png");
...
if(parseInt(results[indexSelected].price)<=parseInt(origPrice)&&notFound==0&&document.getElementById('detailOutWrap') == null) {
   ...
   $('body').before('<div id="detailOutWrap"><div id="detailInWrap"><a target="_blank" href="http://compare.buyhatke.com" title="Visit Buyhatke"><img id="details_logo" src="http://compare.buyhatke.com/images/logo_small.png"></a><div id="details">Hurray !  Massive savings found. This product is available for <span id="detail_cost"><img src="http://compare.buyhatke.com/images/rupeeK.png"> ' + results[indexSelected].price + '</span> at <span id="detail_store">' + siteName2 + '</span><a style="display:inline!important;" href="' + results[indexSelected].link + '" target="_blank"><input type="button" value=" BUY IT NOW" style="display:inline!important;"></a>or<div class="drop_down" id="compare_now" onmouseover="cancel=true;">COMPARE PRICES<div class="drop_down_symbol"></div><div id="dd_menu"><head><div id="dd_menu_header">Showing <span>' + results.length + '</span> results</div></head>' + string + string2);
...
blinker();
 	 ...
 	 $('body').before('<div id="detailOutWrap"><div id="detailInWrap"><div id="details">Hurray !  Massive savings found. Click to know more <a href="' + url + '" target="_blank"><input type="button" value=" COMPARE PRICES" style="display:inline!important;"></a><a target="_blank" href="https://www.facebook.com/sharer/sharer.php?u=http%3A%2F%2Fextension.buyhatke.com" id="notify_fshare"></a></div><a href="#" id="detailClose" >x</a></div></div>');
   $('body').css("margin-top", "45px");
