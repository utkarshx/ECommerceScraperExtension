...
function reportPurchase(){
  ...
  if(curURL.split('yebhi.com/order/payment').length>1){
    chrome.runtime.sendMessage({processDONE: "Yebhi"}, function(response) {
    });
  }
}
...
function preProcessor(i, coupon){
  ...
  $('.prmocode_textbox').val(coupon);
  document.getElementsByClassName("prmocode_applybtn")[0].click();
...
max = -111111;
ind_req = 1000;
for(i=0;i<parseFloat(getCookie("doneTill"))+3;i++){
  varName = "savingsYebhi" + i;
  curSaving = getCookie(varName);
  curSaving = parseFloat(curSaving);
  console.log(i + " " + curSaving);
  if(max < curSaving){
    max = curSaving;
    ind_req = i;
  }
...
var parameters = "ext_id=" + ext_id + "&auth_val=" + ext_auth + "&pos=6";
...
function checkSavings(){
  ...
  if($('.discountamt').length>0){
    savings = $('.discountamt').text();
    savings = parseFloat(savings);
    varName = "savingsYebhi" + getCookie("doneTill");
...
if(curURL.split('/order/payment.aspx').length>1){
  ...
  if($('#addBook').length>0){
  $('#addBook').after("<a id='couponClick' href='javascript:void();'><img style='margin-left:65px;' src='" + imgURL + "'></a>");
  addToDOM();
...
button.addEventListener("click", function(){
  ...
  document.getElementById('bhWidget').innerHTML = '<div class="line fk-font-12" style="margin-bottom: 4px;">Thank You ! We appreciate your motive to save energy :)</div>';
      ...
      $('.item_size').find('input').before(strToDisp);
      var button = document.getElementById("removeMe2");
button.addEventListener("click", function(){
  removeAlert();
  document.getElementById('bhWidget').innerHTML = '<div class="line fk-font-12" style="margin-bottom: 4px;">Thank You ! We appreciate your motive to save energy :)</div>';
...
function checkMargin(){
  if(resultsShown){
    $('#floatmenubar').css("top", "121px");
    $('#floatmenubar').css("position", "relative");
  }
  setTimeout("checkMargin()",100);
}
...
var prod = $('.pdnamemain').find('h1').text();
...
if($('.PDcenterimagesdiv').length>0){
var image = $('.PDcenterimagesdiv').find('img:eq(0)').attr('src');
}
else {
  var image = "";
}
var parameters =  encodeURIComponent(prod) + "~*~*" + myPrice + "~*~*" + encodeURIComponent(image) + "~*~*" + encodeURIComponent(url) + "~*~*11" ;
...
resultsShown = 0;
$(window).on("load scroll resize",function(){$('#floatmenubar').css({'position':'relative','top':'0'}).prev().css({'margin-bottom':'0'});});
if($('.bredcrummaindiv').text().trim().split("Apparels").length>1 || $('.bredcrummaindiv').text().trim().split("Shoes").length>1 || $('.bredcrummaindiv').text().trim().split("Bags").length>1 || $('.bredcrummaindiv').text().trim().split("Watches").length>1 || $('.bredcrummaindiv').text().trim().split("Jewellery").length>1 || $('.bredcrummaindiv').text().trim().split("Timepieces").length>1 || $('.bredcrummaindiv').text().trim().split("Innerwear").length>1){
  isApparels = false;
...
if(isApparels){
   var name = $('.pdnamemain').find('h1').text();
   console.log(name);
...
function redirect(){
 ...
 var imgURL = chrome.extension.getURL("yebhi2.png");

if($('.item_size').length>0){
  if(flagToDisp==0){
$('.item_size').find('input').before('<div id="bhWidget"><a id="addWatchList" style="margin-top: 10px;margin-left:-10px;" alt="Add to Watch List" title="Add to BuyHatke Watch List and get notifications on price drop" href="javascript:void();" class="fk-inline-block buy-btn fksk-buy-btn"><img src=' + imgURL2 +'></a></div>');
...
$('.item_size').find('input').after('<a href=' + url +' target="_blank" title="Compare via Compare Hatke"><img src=' + imgURL + ' title="Compare via Compare Hatke" style="margin-left: 6px;"></a>');
