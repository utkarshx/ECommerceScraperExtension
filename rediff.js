...
var title = $('.detailed-description-head').text().trim();
origProd = title;
title = title.split(" ");
var title2 = title.join("-");
var urlToFollow = url + title2;
if($('.bigBold').length>0){
var price = $('.bigBold').text().trim().split("Rs.")[1].trim();
}
else {
  price = 10000;
}

var imgURL = chrome.extension.getURL("rediffbooks.png");
$('.btn-holder').find('a:first').after('<a target= "blank_" title="Compare via Compare Hatke" href="' + urlToFollow + '"><img src=' + imgURL + ' title="Compare via Compare Hatke" style="margin-left: 28px;"></a>');
